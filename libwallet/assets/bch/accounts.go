...
func (asset *Asset) GetAccountsRaw() (*sharedW.Accounts, error) {
	...
	resp, err := asset.Internal().BCH.Accounts(GetScope())
	if err != nil {
		return nil, err
	}
...
func (asset *Asset) GetAccountBalance(accountNumber int32) (*sharedW.Balance, error) {
	...
	balance, err := asset.Internal().BCH.CalculateAccountBalances(uint32(accountNumber), asset.RequiredConfirmations())
	if err != nil {
		return nil, err
	}
...
// LockUnspent.
func (asset *Asset) lockedAmount() (bchutil.Amount, error) {
	lockedOutpoints := asset.Internal().BCH.LockedOutpoints()
	var sum int64
...
func (asset *Asset) SpendableForAccount(account int32) (int64, error) {
	if !asset.WalletOpened() {
		return -1, utils.ErrBCHNotInitialized
	}

	bals, err := asset.Internal().BCH.CalculateAccountBalances(uint32(account), asset.RequiredConfirmations())
	if err != nil {
...
func (asset *Asset) UnspentOutputs(account int32) ([]*sharedW.UnspentOutput, error) {
	...
	for _, utxo := range unspents {
		...
		amount, _ := bchutil.NewAmount(utxo.Amount)
...
func (asset *Asset) NextAccount(accountName string) (int32, error) {
	...
	accountNumber, err := asset.Internal().BCH.NextAccount(GetScope(), accountName)
	if err != nil {
		return -1, err
	}
...
func (asset *Asset) RenameAccount(accountNumber int32, newName string) error {
	...
	err := asset.Internal().BCH.RenameAccount(GetScope(), uint32(accountNumber), newName)
	if err != nil {
...
func (asset *Asset) AccountNameRaw(accountNumber uint32) (string, error) {
	...
	return asset.Internal().BCH.AccountName(GetScope(), accountNumber)
}
...
func (asset *Asset) AccountNumber(accountName string) (int32, error) {
	...
	accountNumber, err := asset.Internal().BCH.AccountNumber(GetScope(), accountName)
...
func (asset *Asset) HasAccount(accountName string) bool {
	if !asset.WalletOpened() {
		log.Error(utils.ErrBCHNotInitialized)
		return false
	}

	_, err := asset.Internal().BCH.AccountNumber(GetScope(), accountName)
	return err == nil
}
