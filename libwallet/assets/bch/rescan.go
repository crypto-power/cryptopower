...
import (
	...
	bchwallet "github.com/dcrlabs/bchwallet/wallet"
...
func (asset *Asset) rescanBlocks(startHeight int32, addrs []bchutil.Address) error {
	if !asset.IsConnectedToBitcoinCashNetwork() {
	...
	errChan := asset.Internal().BCH.SubmitRescan(job)
...
func (asset *Asset) forceRescan() {
	wdb := asset.Internal().BCH.Database()
	err := walletdb.Update(wdb, func(dbtx walletdb.ReadWriteTx) error {
		...
		if asset.IsRestored && !asset.ContainsDiscoveredAccounts() {
			...
			block := asset.Internal().BCH.ChainParams().GenesisBlock
			...
			err := asset.Internal().BCH.Manager.SetBirthdayBlock(ns, bs, true)
			if err != nil {
		...
		return asset.Internal().BCH.Manager.SetSyncedTo(ns, nil)
	})
	if err != nil {
...
func (asset *Asset) updateAssetBirthday() {
	...
	if len(txs) > 0 {
		...
		syncedTo := asset.Internal().BCH.Manager.SyncedTo()
	...
	err = walletdb.Update(asset.Internal().BCH.Database(), func(dbtx walletdb.ReadWriteTx) error {
		ns := dbtx.ReadWriteBucket(wAddrMgrBkt)
		err := asset.Internal().BCH.Manager.SetBirthday(ns, block.Timestamp)
		if err != nil {
			return err
		}
		...
		// the most optimum birthday block. On premature exit if the
		// optimum will be available by then, its also set automatically.
		return asset.Internal().BCH.Manager.SetBirthdayBlock(ns, *block, true)
	})
...
func (asset *Asset) getBirthdayBlock() (int32, bool, error) {
	...
	var isverified bool
	err := walletdb.View(asset.Internal().BCH.Database(), func(dbtx walletdb.ReadTx) error {
		ns := dbtx.ReadBucket(wAddrMgrBkt)
		b, ok, err := asset.Internal().BCH.Manager.BirthdayBlock(ns)
...
// updateSyncedToBlock is used to update syncedTo block. Sometimes bchwallet might
// miss the trigger event to update syncedTo block so the update is done here
// regardless thus avoid handling the possible scenario where bchwallet might miss
...
func (asset *Asset) updateSyncedToBlock(height int32) {
	...
	err := walletdb.Update(asset.Internal().BCH.Database(), func(dbtx walletdb.ReadWriteTx) error {
		...
		return asset.Internal().BCH.Manager.SetSyncedTo(addrmgrNs, bs)
	})
	if err != nil {
