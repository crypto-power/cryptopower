...
func (asset *Asset) prepareChain() error {
	...
	log.Debug("Starting native BCH wallet sync...")
...
func (asset *Asset) loadChainService() (chainService *neutrino.ChainService, err error) {
	...
	chainService, err = neutrino.NewChainService(neutrino.Config{
		...
		Database:      asset.GetWalletDataDb().BCH,
...
func (asset *Asset) startSync() error {
	...
	asset.Internal().BCH.SynchronizeRPC(asset.chainClient)
...
// IsConnectedToBitcoinCashNetwork returns true if the wallet is connected to the
// bitcoin cash network.
func (asset *Asset) IsConnectedToBitcoinCashNetwork() bool {
	asset.syncData.mu.RLock()
	defer asset.syncData.mu.RUnlock()
...
func (asset *Asset) SpvSync() (err error) {
	...
	go func() {
		...
		if err != nil {
			log.Warn("error occurred when starting BCH sync: ", err)
		}
	}()
