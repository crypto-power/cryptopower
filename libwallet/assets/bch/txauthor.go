...
type TxAuthor struct {
	...
	txSpendAmount     bchutil.Amount // Equal to fee + send amount
...
func (asset *Asset) ComputeTxSizeEstimation(dstAddress string, utxos []*sharedW.UnspentOutput) (int, error) {
	...
	output, err := txhelper.MakeBCHTxOutput(dstAddress, sendAmount, asset.chainParams)
	if err != nil {
...
func (asset *Asset) TotalSendAmount() *sharedW.Amount {
	...
	return &sharedW.Amount{
		...
		CoinValue: bchutil.Amount(totalSendAmountLitoshi).ToBCH(),
	}
}
...
func (asset *Asset) EstimateFeeAndSize() (*sharedW.TxFeeAndSize, error) {
	...
	sendAmount := bchutil.Amount(asset.TotalSendAmount().UnitValue)
	...
	if unsignedTx.ChangeIndex >= 0 {
		...
		change = &sharedW.Amount{
			...
			CoinValue: AmountBCH(txOut.Value),
		}
	}
	...
	// estimatedSize := feeToSpend.ToBCH() / fallBackFeeRate.ToBCH()
...
func (asset *Asset) EstimateMaxSendAmount() (*sharedW.Amount, error) {
	...
	return &sharedW.Amount{
		...
		CoinValue: bchutil.Amount(maxSendableAmount).ToBCH(),
	}, nil
}
...
func (asset *Asset) Broadcast(privatePassphrase, transactionLabel string) ([]byte, error) {
	...
	err = asset.Internal().BCH.Unlock([]byte(privatePassphrase), lock)
	if err != nil {
	...
	err = asset.Internal().BCH.PublishTransaction(msgTx)
	return nil, utils.TranslateError(err)
}
...
func (asset *Asset) constructTransaction() (*txauthor.AuthoredTx, error) {
	...
	setFeeRate := bchutil.Amount(asset.GetUserFeeRate().ToInt())
	var sendMax bool
	...
	for _, destination := range asset.TxAuthoredInfo.destinations {
		...
		if destination.SendMax {
			...
			changeSource, err = txhelper.MakeBCHTxChangeSource(destination.Address, asset.chainParams)
			if err != nil {
			...
			output, err := txhelper.MakeBCHTxOutput(destination.Address, destination.UnitAmount, asset.chainParams)
			if err != nil {
	...
	if changeSource == nil {
		// bchwallet should ordinarily handle cases where a nil changeSource
...
func (asset *Asset) changeSource() (*txauthor.ChangeSource, error) {
	if asset.TxAuthoredInfo.changeAddress == "" {
		...
		address, err := asset.Internal().BCH.NewChangeAddress(changeAccount, GetScope())
		if err != nil {
	...
	changeSource, err := txhelper.MakeBCHTxChangeSource(asset.TxAuthoredInfo.changeAddress, asset.chainParams)
	if err != nil {
		log.Errorf("constructTransaction: error preparing change source: %v", err)
...
func (asset *Asset) makeInputSource(outputs []*sharedW.UnspentOutput, sendMax bool) txauthor.InputSource {
	var (
		...
		inputValues = make([]bchutil.Amount, 0, len(outputs))
	...
	for _, output := range outputs {
		...
		totalInputValue += bchutil.Amount(output.Amount.(Amount))
		pkScripts = append(pkScripts, script)
		inputValues = append(inputValues, bchutil.Amount(output.Amount.(Amount)))
	...
	return func(target bchutil.Amount) (bchutil.Amount, []*wire.TxIn, []bchutil.Amount, [][]byte, error) {
