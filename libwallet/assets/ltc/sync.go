	...
	for {
		...
		case n := <-notes.C:
			for _, block := range n.AttachedBlocks {
				...
				// Spamming this before sync causes a lot of unneeded balance updates.
				if asset.IsSynced() {
			...
			if len(n.AttachedBlocks) > 0 {
				select {
				case <-t.C:
					asset.updateSyncProgress(n.AttachedBlocks[len(n.AttachedBlocks)-1].Height)
				default:
				}
			}
...
func (asset *Asset) waitForSyncCompletion() {
	...
	for {
		...
		case <-t.C:
			asset.syncData.mu.RLock()
			height := asset.syncData.bestBlockheight
			asset.syncData.mu.RUnlock()

			asset.updateRescanProgress(height)

			if asset.chainClient.IsCurrent() {
				asset.rescanFinished(height)
				asset.syncData.mu.Lock()
				asset.syncData.synced = true
				asset.syncData.syncing = false
				asset.syncData.mu.Unlock()
