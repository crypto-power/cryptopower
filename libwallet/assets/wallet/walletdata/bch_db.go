...
var _ walletdb.DB = (*BCHDB)(nil)

func (db *BCHDB) beginTx(writable bool) (*BCHTX, error) {
	boltTx, err := db.Bolt.Begin(writable)
	if err != nil {
		return nil, err
	}
	return &BCHTX{boltTx: boltTx}, nil
}

// BeginReadTx opens a database read transaction.
func (db *BCHDB) BeginReadTx() (walletdb.ReadTx, error) {
	return db.beginTx(false)
}

// BeginReadWriteTx opens a database read+write transaction.
func (db *BCHDB) BeginReadWriteTx() (walletdb.ReadWriteTx, error) {
...
func (db *BCHDB) Copy(w io.Writer) error {
...
func (db *BCHDB) Close() error {
	return db.Bolt.Close()
}

// PrintStats returns all collected stats pretty printed into a string.
func (db *BCHDB) PrintStats() string {
	return "---"
}
...
func (db *BCHDB) View(f func(tx walletdb.ReadTx) error, reset func()) error {
...
func (db *BCHDB) Update(f func(tx walletdb.ReadWriteTx) error, reset func()) error {
	// We don't do any retries with bolt so we just initially call the reset
	// function once.
	reset()
...
var _ walletdb.ReadWriteTx = (*BCHTX)(nil)

// ReadBucket opens the root bucket for read only access.  If the bucket
// described by the key does not exist, nil is returned.
func (tx *BCHTX) ReadBucket(key []byte) walletdb.ReadBucket {
	return tx.ReadWriteBucket(key)
}

// ForEachBucket will iterate through all top level buckets.
func (tx *BCHTX) ForEachBucket(fn func(key []byte) error) error {
...
func (tx *BCHTX) ReadWriteBucket(key []byte) walletdb.ReadWriteBucket {
	boltBucket := tx.boltTx.Bucket(key)
	if boltBucket == nil {
		return nil
	}
	return &BCHBucket{boltBucket: boltBucket}
}

// CreateTopLevelBucket creates the top level bucket for a key if it
// does not exist.  The newly-created bucket it returned.
func (tx *BCHTX) CreateTopLevelBucket(key []byte) (walletdb.ReadWriteBucket, error) {
	boltBucket, err := tx.boltTx.CreateBucketIfNotExists(key)
	if err != nil {
		return nil, err
	}
	return &BCHBucket{boltBucket: boltBucket}, nil
}
...
// instead of a bucket.
func (tx *BCHTX) DeleteTopLevelBucket(key []byte) error {
...
// implementations.
func (b *BCHBucket) ForEach(fn func(k, v []byte) error) error {
...
// data copies and allows support for memory-mapped database
// implementations.
func (b *BCHBucket) Get(key []byte) []byte {
	return b.boltBucket.Get(key)
}

// NestedReadWriteBucket retrieves a nested bucket with the given key.
// Returns nil if the bucket does not exist.
func (b *BCHBucket) NestedReadWriteBucket(key []byte) walletdb.ReadWriteBucket {
...
// implementation.
func (b *BCHBucket) CreateBucket(key []byte) (walletdb.ReadWriteBucket, error) {
...
func (b *BCHBucket) CreateBucketIfNotExists(key []byte) (walletdb.ReadWriteBucket, error) {
...
func (b *BCHBucket) DeleteNestedBucket(key []byte) error {
...
func (b *BCHBucket) Put(key, value []byte) error {
...
func (b *BCHBucket) Delete(key []byte) error {
	return b.boltBucket.Delete(key)
}

func (b *BCHBucket) ReadCursor() walletdb.ReadCursor {
...
// order.
func (b *BCHBucket) ReadWriteCursor() walletdb.ReadWriteCursor {
	return b.boltBucket.Cursor()
}

// Tx returns the bucket's transaction.
func (b *BCHBucket) Tx() walletdb.ReadWriteTx {
	return &BCHTX{
		b.boltBucket.Tx(),
	}
}
