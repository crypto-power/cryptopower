...
// SetParent sets the parent window navigator Since DexServerSelector doesn't
// implement the Page interface thus its parent can't be accessed via
// pageNavigation interface.
func (ds *DexServerSelector) SetParent(parent app.WindowNavigator) {
	ds.OnAttachedToNavigator(parent)
}
