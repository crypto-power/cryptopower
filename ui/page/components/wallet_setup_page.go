...
func (pg *CreateWallet) HandleUserInteractions() {
	...
	// asset type dropdown action
	if pg.assetTypeDropdown.Changed() {
	...
	if pg.restoreBtn.Clicked() && pg.validRestoreWalletInputs() {
		...
		pg.ParentWindow().Display(NewRestorePage(pg.Load, pg.walletName.Editor.Text(), getAssetType(pg.assetTypeDropdown.Selected()), afterRestore))
	}
