...
// are irrelevant when the page is NOT displayed. NOTE: The page may be
// re-displayed on the app's window, in which case OnNavigatedTo() will be
// called again. This method should not destroy UI components unless they'll be
// recreated in the OnNavigatedTo() method.
// Part of the load.Page interface.
...
func (pg *WalletSelectorPage) startSyncing(wallet sharedW.Asset, unlock load.NeedUnlockRestore) {
	// Watchonly wallets do not have any password and do not need one.
	...
	if pg.isConnected {
		// Once network connection has been established, proceed to start the
		// wallet sync.
